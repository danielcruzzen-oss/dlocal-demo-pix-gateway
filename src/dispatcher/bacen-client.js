...
class BacenDispatcher {
  constructor(opts = {}) {
    this.endpoint = opts.endpoint || config.bacen.endpoint;
    this.timeoutMs = opts.timeoutMs || config.bacen.timeout_ms;
    this.retryPolicy = opts.retryPolicy || config.bacen.retry_policy;
    ...
    try {
      for (let attempt = 1; attempt <= this.retryPolicy.max_attempts; attempt++) {
        try {
          ...
          throw new Error(`BACEN rejected: ${response.status}`);
        } catch (err) {
...
module.exports = { BacenDispatcher };
