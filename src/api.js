...
const { RedBancariaClient } = require('./dispatcher/red-bancaria-client');
const { Ledger } = require('./ledger/store');

const client = new RedBancariaClient();
...
async function handleTransfer(req, res) {
  ...
  try {
    ...
    const result = await client.dispatch(transaction);
    ...
    return res.status(502).json({ error: 'red_bancaria_uy_dispatch_failed' });
  }
}
